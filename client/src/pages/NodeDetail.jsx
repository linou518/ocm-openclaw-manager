...
import AddBotModal from '../components/enhanced-AddBotModal-with-health';
...
function NodeDetail() {
  ...
  const handleDeleteBot = async (botId, botName) => {
    ...
    try {
      const res = await fetch(`/api/bots/${botId}/real-delete`, {
        method: 'DELETE',
      });
  ...
  const handleSyncAgents = async () => {
    ...
    try {
      ...
      if (res.ok) {
        alert(`✅ Agent同步成功: ${result.message}`);
        // 重新获取Bots信息
        await fetchBots();
      } else {
        alert(`❌ Agent同步失败: ${result.error}`);
      }
    } catch (error) {
      alert(`❌ Agent同步失败: ${error.message}`);
    } finally {
      setActionLoading(false);
    }
  };
  ...
  const handleBotStart = async (botId) => {
    setActionLoading(true);
    try {
      const res = await fetch(`/api/bots/${botId}/deploy-and-start`, {
        method: 'POST'
      });
      const result = await res.json();
  ...
  const handleStartAllBots = async () => {
    setActionLoading(true);
    try {
      const res = await fetch(`/api/nodes/${id}/bots/deploy-all`, {
        method: 'POST'
      });
      const result = await res.json();
      if (res.ok && result.success) {
        alert(`✅ ${result.message}\n\n已启动Bot:\n${result.deployed_bots?.map(b => `• ${b.name}`).join('\n') || '无'}`);
        fetchBots();
      } else {
        alert('❌ 批量启动失败: ' + result.error);
      }
    } catch (error) {
  ...
  // handleClearAllBots 已移除 - 使用智能单个删除代替
  ...
  const showImplementation = (type, itemId) => {
    const implementations = {
      backup: {
        feature: `备份节点 ${id}`,
        description: 'SSH 连接到节点，打包配置和 workspace，上传到 GitHub',
        steps: [
          `SSH 连接到 ${node?.host}:${node?.port}`,
          '执行 ocm-agent backup (节点上的 shell 脚本)',
          '打包 ~/.openclaw/config/ 配置文件',
          '打包 ~/.openclaw/workspace/ 工作区',
          '生成 metadata.json (时间、文件数、大小)',
          'SCP 下载到 Master 的 ocm-backups/nodes/${node?.id}/',
        ...
        note: '备份时间取决于 workspace 大小，通常 10-30 秒'
      },
      restore: {
        feature: `还原节点 ${id}`,
        description: '从 GitHub 恢复配置到节点，需先选择备份点',
        steps: [
          '用户选择要还原的备份（通过 tag 或 commit hash）',
          '⚠️ 确认提示："此操作会覆盖当前配置，是否继续？"',
          '创建还原前快照 (rollback-pre)',
          'git clone 或 pull ocm-backups 仓库',
          `从 nodes/${id}/ 目录提取文件`,
          'SSH 连接到节点',
          'systemctl stop openclaw (停止服务)',
          'SCP 上传文件到 ~/.openclaw/',
          'chmod +x ~/.openclaw/bin/* (恢复权限)',
          'systemctl start openclaw (重启服务)',
          '等待 30s 健康检查',
          '触发智力测试验证效果',
          '记录还原操作到审计日志'
        ],
        ...
        note: '还原后会自动触发智力测试，确保节点恢复正常'
      },
      restart: {
        feature: `重启节点 ${id}`,
        description: '通过 SSH 重启 OpenClaw 服务',
        steps: [
          `SSH 连接到 ${node?.host}`,
          '执行 systemctl restart openclaw',
          '等待服务启动（最多 30s）',
          '检查服务状态：systemctl is-active openclaw',
          '如果失败，尝试 kill 进程后重启',
          '更新节点状态为 online/offline',
          '记录到事件日志',
          '发送通知'
        ],
        tech: ['ssh2', 'systemctl'],
        api: { method: 'POST', endpoint: `/api/nodes/${id}/restart` },
        note: '重启会中断正在进行的对话，谨慎操作'
      },
      test: {
        feature: `智力测试 ${id}`,
        description: '发送测试题到节点，由 LLM-as-Judge 评分',
        steps: [
          '从测试题库随机抽取 5 道题（每维度 1 道）',
          '通过节点的 Bot 发送测试消息',
          '记录响应时间和内容',
          'LLM Judge (GPT-4) 评分：',
          '  - 记忆一致性 (检查 MEMORY.md 引用) /20',
          '  - 逻辑推理 (多步推理题) /20',
          '  - 工具使用 (需要调用工具的任务) /20',
          '  - 回复质量 (语言流畅度、准确性) /20',
          '  - 人格一致性 (与 SOUL.md 对比) /20',
          '汇总总分 /100',
          '如果 < 80 分，触发告警',
          '如果 < 60 分，建议自动回滚',
          '保存评分到数据库',
        ...
        note: '测试会消耗约 $0.05-0.10 的 AI 费用'
      },
      restoreVersion: {
        feature: `还原到此版本`,
        description: '快速还原到选定的备份版本',
        steps: [
          '读取备份的 git_commit 或 git_tag',
          '调用还原 API',
          '(同 restore 流程)'
        ],
      ...
      restartBot: {
        feature: `重启 Bot ${itemId}`,
        description: 'SSH 重启节点上的 Bot 进程',
        steps: [
          `SSH 连接到 ${node?.host}`,
          '查找 Bot 进程：ps aux | grep openclaw | grep <workspace>',
          'kill -15 <pid> (优雅停止)',
          '等待 5s',
          '如果未停止，kill -9 <pid> (强制停止)',
          '重新启动 Bot: openclaw start --workspace <path>',
          '更新 Bot 状态为 running',
          '记录操作'
        ],
        tech: ['ssh2', 'ps', 'kill', 'openclaw-cli'],
        api: { method: 'POST', endpoint: `/api/bots/${itemId}/restart` },
        note: 'Bot 会话会丢失，需要用户重新发起对话'
      },
      stopBot: {
        feature: `停止 Bot ${itemId}`,
        description: 'SSH 停止节点上的 Bot 进程',
        steps: [
          `SSH 连接到 ${node?.host}`,
          '查找 Bot 进程',
          'kill -15 <pid>',
        ...
        note: '停止后需要手动重启或通过 OCM 重启'
      },
      addKey: {
        feature: '添加 API Key',
        description: '输入 Key 信息，存入数据库，并同步到节点配置',
        steps: [
          '用户输入：provider, key_name, api_key, monthly_limit',
          '插入到数据库 api_keys 表',
          'SSH 连接到节点',
          '读取 ~/.openclaw/config.yaml',
          '添加或更新 keys 字段',
          '写回配置文件',
          'systemctl reload openclaw (热重载)',
          '验证 Key 有效性 (可选)',
          '发送通知'
        ],
        tech: ['sqlite3', 'ssh2', 'yaml', 'systemctl'],
        api: { method: 'POST', endpoint: `/api/nodes/${id}/keys` },
        note: 'Key 会加密存储，传输时使用 SSH'
      }
    };
    ...
    <div className="space-y-6 pb-20 md:pb-0">
      ...
      <div className="bg-gray-800 rounded-lg border border-gray-700 p-6 shadow-lg">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div>
            <div className="text-sm text-gray-400">OpenClaw 版本</div>
            <div className="text-xl font-semibold text-white">{node.openclaw_version || '未安装'}</div>
          </div>
        </div>
      </div>
        ...
        <div className="p-4 md:p-6">
          ...
          {tab === 'overview' && (
            <div className="space-y-4">
              <div>
                <h3 className="font-semibold text-white mb-2">基本信息</h3>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                  ...
                  <button                    onClick={() => setConfirmDialog({                      isOpen: true,                      action: 'delete',                      data: null                    })}                    disabled={actionLoading}                    className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-sm rounded transition-colors disabled:opacity-50"                  >                    🗑️ 退役                  </button>
                </div>
              </div>
            </div>
          )}
                ...
                <table className="w-full text-sm">
                  <thead className="bg-gray-900 text-xs text-gray-400 uppercase border-b border-gray-700">
                    <tr>
                      ...
                      <th className="px-3 md:px-4 py-2 text-left">名称</th>
                      <th className="px-3 md:px-4 py-2 text-left hidden md:table-cell">Key</th>
                      <th className="px-3 md:px-4 py-2 text-center">状态</th>
                      <th className="px-3 md:px-4 py-2 text-right hidden lg:table-cell">用量/限额</th>
                      <th className="px-3 md:px-4 py-2 text-right">操作</th>
                    </tr>
                  </thead>
                    ...
                    <div key={bot.id} className="bg-gray-900 border border-gray-700 rounded-lg p-6 shadow-lg">
                          ...
                          <div className={`font-semibold text-lg ${
                            ...
                            {isSelected && latestMemory ? (
                              <>
                                ...
                                {latestMemory.health_status === 'warning' && ' ⚠️'}
                              </>
                            ) : '-'}
                          </div>
                        </div>
                        ...
                        <div className="mt-3 pt-3 border-t border-gray-200 space-y-3">
                              ...
                              {botSessions.slice(0, 3).map(session => (
                                <div key={session.id} className="flex items-center justify-between text-xs bg-gray-50 px-2 py-1 rounded">
                                  <span className="flex items-center space-x-1">
                                    <span>{session.session_type === 'main' ? '💬' : session.session_type === 'cron' ? '⏰' : '🔀'}</span>
                          ...
                          {latestMemory && latestMemory.health_status === 'warning' && (
                            <div className="text-xs p-2 bg-yellow-50 border border-yellow-200 rounded">
                              <div className="font-medium text-yellow-800 mb-1">⚠️ Memory Warning</div>
                      ...
                      <div className="flex justify-between items-center mt-4 pt-3 border-t border-gray-700">
                        <div className="text-xs text-gray-400">
                          最后更新: {new Date(bot.updated_at).toLocaleString('zh-CN')}
                        </div>
                        <div className="flex space-x-2">
                          <button
          ...
          {tab === 'skills' && (
            <div className="space-y-4">
              ...
              {skills.length === 0 && (
                <div className="text-center py-8 text-gray-400">暂无Skills</div>
              )}
            </div>
          )}
          ...
          {tab === 'config' && (
            <div className="space-y-4">
              ...
              {gatewayConfig && gatewayConfig.node_id ? (
                    ...
                    <div className="flex flex-wrap gap-2">
                      {['telegram', 'discord', 'whatsapp', 'slack'].map(ch => {
                        ...
                        return (
                          ...
                          >
                            {ch.charAt(0).toUpperCase() + ch.slice(1)} {enabled ? '✅' : '❌'}
                          </div>
                        );
                      })}
                    </div>
                  ...
                  <div>
                    ...
                    <div className="flex items-center space-x-3">
                      <div className={`px-3 py-2 rounded text-sm font-medium ${
                        ...
                        {gatewayConfig.heartbeat_enabled ? '✅ Enabled' : '❌ Disabled'}
                      </div>
                ...
                <div className="text-center py-8 text-gray-400">配置未加载</div>
              )}
            </div>
          )}
          ...
          {tab === 'backups' && (
            <div className="space-y-2">
              {backups.length === 0 ? (
                <div className="text-center py-8 text-gray-400">暂无备份</div>
              ) : (
                  ...
                  <div key={backup.id} className="border border-gray-200 rounded p-3">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center space-x-2">
                        ...
                        {backup.is_stable && (
                          <span className="text-xs px-2 py-1 bg-green-100 text-green-800 rounded">✅ stable</span>
                        )}
                        {backup.score && (
                    ...
                    <div className="text-xs text-gray-500 space-y-1">
                      ...
                      <div>{backup.file_count} 文件 · {(backup.total_size / 1024 / 1024).toFixed(2)} MB</div>
                    </div>
                    <button
              ...
              {backups.length > pageSize && (
                <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-200 text-sm text-gray-600">
                  <span>共 {backups.length} 条，第 {backupsPage}/{Math.ceil(backups.length / pageSize)} 页</span>
          ...
          {tab === 'scores' && (
            <div className="space-y-2">
              {scores.length === 0 ? (
                <div className="text-center py-8 text-gray-400">暂无评分</div>
              ) : (
              ...
              {scores.length > pageSize && (
                <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-200 text-sm text-gray-600">
                  <span>共 {scores.length} 条，第 {scoresPage}/{Math.ceil(scores.length / pageSize)} 页</span>
                  <div className="flex gap-2">
                    <button 
          ...
          {tab === 'events' && (
            <div className="space-y-2">
              ...
              {events.length > pageSize && (
                <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-200 text-sm text-gray-600">
                  <span>共 {events.length} 条，第 {eventsPage}/{Math.ceil(events.length / pageSize)} 页</span>
                  <div className="flex gap-2">
                    <button 
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          ...
          } else if (confirmDialog.action === 'start-bot') {
            handleBotStart(confirmDialog.data.botId);
          } else {
            handleNodeAction(confirmDialog.action);
          }
        }}
        title={
          confirmDialog.action === 'backup' ? '确认备份' :
          confirmDialog.action === 'restart' ? '确认重启' :
          confirmDialog.action === 'test' ? '确认测试' :
          confirmDialog.action === 'delete' ? '确认删除节点' :
          confirmDialog.action === 'restart-bot' ? '确认重启 Bot' :
          confirmDialog.action === 'start-bot' ? '确认启动 Bot' :
          '确认操作'
        }
        message={
          confirmDialog.action === 'backup' ? `确定要备份节点 ${id} 吗？` :
          confirmDialog.action === 'restart' ? `确定要重启节点 ${id} 吗？这会中断正在进行的会话。` :
          confirmDialog.action === 'test' ? `确定要对节点 ${id} 进行智力测试吗？预计需要 1-2 分钟。` :
          confirmDialog.action === 'delete' ? `⚠️ 警告：即将删除节点 ${id}！
...
此操作不可逆，确定要继续吗？` :
          confirmDialog.action === 'restart-bot' ? `确定要重启 Bot ${confirmDialog.data?.botName} 吗？` :
          confirmDialog.action === 'start-bot' ? `确定要启动 Bot ${confirmDialog.data?.botName} 吗？启动后Bot将开始响应Telegram消息。` :
          '确定要执行此操作吗？'
        }
        confirmText={
          confirmDialog.action === 'delete' ? '确认删除' :
          confirmDialog.action === 'restart' || confirmDialog.action === 'restart-bot' ? '确认重启' :
          confirmDialog.action === 'start-bot' ? '确认启动' :
          confirmDialog.action === 'test' ? '开始测试' :
          '确认'
        }
        confirmColor={
          confirmDialog.action === 'delete' ? 'red' :
          confirmDialog.action === 'restart' || confirmDialog.action === 'restart-bot' ? 'red' :
          confirmDialog.action === 'start-bot' ? 'green' : 'blue'
        }

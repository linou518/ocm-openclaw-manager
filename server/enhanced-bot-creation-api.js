...
class EnhancedBotCreationAPI {
    ...
    async checkBotReady(req, res) {
        try {
console.log("🔍 增强API接收到bot-ready请求:", req.params.nodeId);
            const { nodeId } = req.params;
            
            // 获取节点信息
            const node = this.db.prepare('SELECT * FROM nodes WHERE id = ?').get(nodeId);
console.log("增强API - 节点数据:", JSON.stringify(node, null, 2));
            if (!node) {
    ...
    async createBotEnhanced(req, res) {
        try {
            ...
            console.log(`🤖 开始创建Bot: ${botData.bot_name} (增强版)`);
            ...
            // 2. Token有效性验证
            const tokenValidation = await this.validateTelegramToken(botData.bot_token);
            if (!tokenValidation.valid) {
                return res.status(400).json({
                    success: false,
                    error: tokenValidation.error,
                    error_type: 'invalid_token',
                    suggestion: '请检查Bot Token是否正确，或从@BotFather重新获取Token'
                });
            }
            
            console.log(`✅ Token验证通过: ${botData.bot_name}`);
            botData.verified_bot_info = tokenValidation.bot_info;
            ...
            if (!nodeId) {
                return res.status(400).json({
                    success: false,
                    error: `未知的目标服务器: ${botData.target_server}`
                });
            }
            ...
            if (!readyCheck.ready) {
                return res.status(400).json({
                    success: false,
                    error: `节点未准备就绪: ${readyCheck.reason}`,
                    node_status: {
                        name: node.name,
                        status: node.status,
            ...
            console.log(`✅ 节点健康检查通过: ${node.name}`);
            ...
            if (!configResult.success) {
                return res.status(500).json({
                    success: false,
                    error: `配置生成失败: ${configResult.error}`
                });
            }
            
            console.log(`✅ Bot配置生成完成: ${configResult.bundle_path}`);
            ...
            if (!deployResult.success) {
                return res.status(500).json({
                    success: false,
                    error: `部署失败: ${deployResult.error}`,
                    config_path: configResult.bundle_path
                });
            }
            
            console.log(`✅ Bot部署完成: ${botData.bot_name}`);
    ...
    async validateTelegramToken(botToken) {
        try {
            console.log('🔍 验证Telegram Bot Token...');
            const response = await axios.get(`https://api.telegram.org/bot${botToken}/getMe`, {
                timeout: 5000
            });
            
            if (response.data && response.data.ok) {
                console.log(`✅ Token验证成功: @${response.data.result.username}`);
                return { 
                    valid: true, 
                    bot_info: response.data.result 
                };
            } else {
                console.log('❌ Token验证失败: API返回失败');
                return { 
                    valid: false, 
                    error: 'Token无效或已过期' 
                };
            ...
            if (error.response && error.response.status === 401) {
                return { 
                    valid: false, 
                    error: 'Token无效或已过期，请检查Bot Token是否正确' 
                };
            }
            return { 
                valid: false, 
    ...
    async isNodeReadyForBot(node) {
        if (!node) {
            return { ready: false, reason: '节点不存在' };
        }
        ...
        if ((node.last_score || 0) < minScore) {
            return { 
                ready: false, 
                reason: `健康分数过低: ${node.last_score || 0}/${minScore}` 
            };
        }
    ...
    async generateBotConfig(botData) {
        try {
            ...
            if (stderr) {
                console.error('配置生成警告:', stderr);
            }
            
            // 提取配置包路径
            const bundlePath = stdout.match(/CONFIG_BUNDLE_PATH:(.+)/)?.[1]?.trim();
            if (!bundlePath) {
                return { success: false, error: '无法获取配置包路径' };
            }
    ...
    async deployBotToNode(bundlePath, node) {
        try {
            console.log(`🚀 开始部署到节点: ${node.name} (${node.host})`);
            
            // 执行部署脚本
            const deployCmd = `cd ${bundlePath} && chmod +x deploy.sh && ./deploy.sh`;
            const { stdout, stderr } = await execAsync(deployCmd, { timeout: 300000 }); // 5分钟超时
    ...
    getProfessions(req, res) {
        const professions = [
            ...
            },
            {
                id: "data-eng", 
                name: "数据工程专家",
                description: "数据管道、ETL、大数据处理",
                icon: "📊",
                skills: ["Python", "SQL", "Spark", "数据建模"]
            },
            {
                id: "general",
                name: "通用助理",
                description: "全能AI助理，适用于各种任务",
                icon: "🤖",
                skills: ["通用对话", "任务协助", "信息查询", "文档处理"]
            }
        ];
